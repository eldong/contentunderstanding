...
- Implementation:
  ...
  2. Set up components:
     ...
     - Else: use `DocIntelligenceExtractor(endpoint=os.getenv("AZURE_DOC_INTEL_ENDPOINT"), key=os.getenv("AZURE_DOC_INTEL_KEY"))`
     - Create `AsyncAzureOpenAI` client from env vars (unless `--mock` — then create mock-compatible stubs)
